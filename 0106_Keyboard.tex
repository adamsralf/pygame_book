...
As a first step, a dictionary\index{Dictionary} of possible directions is created in \texttt{config.py}. These directions are managed as \texttt{Vector2} objects \myindex{pyg}{\texttt{math}!\texttt{Vector2D}}, since they are easier to use for mathematical operations.
...
\lstsource{SRC/00 Introduction/06 Keyboard/keyboard/v01/keyboard.py}{9}{26}{python}{Control Direction by Keys (2), Class \texttt{Defender}}{srcTastatur00a} 
...
\randnotiz{event.mod}\index{event.mod} is set. This bitmask must be checked using the \gls{binaryand} operation.

In our example, this is done in \zeiref{srcTastatur0102} using \texttt{KMOD\_LSHIFT}\randnotiz{KMOD\_LSHIFT KMOD\_RSHIFT}\myindex{pyg}{\texttt{KMOD\_LSHIFT}} and in \zeiref{srcTastatur0103} using \texttt{KMOD\_RSHIFT}\myindex{pyg}{\texttt{KMOD\_RSHIFT}}.
...
As a complement, it should also be noted that \texttt{KMOD\_NONE}\randnotiz{KMOD\_NONE}\myindex{pyg}{\texttt{KMOD\_NONE}} can be used to check whether no modifier key is pressed at all (see \zeiref{srcTastatur0101}).
...
Just as a small exercise, let us write a simple program that visualizes a key press (see \abbref[vref]{picKeyboard00}). For this purpose, I want to define the keyboard layout in \texttt{config.py} so that it can be easily adapted to different variants. Here is a simple U.S. keyboard layout.
...
The infix \gls{infix} \emph{just} therefore refers to the time span within two frames.

So, if you want to check whether the key \keys{K} is currently pressed, you can simply write something like:
...
With this approach, actions inside the loop can react immediately to a key being held down -- for example, \emph{walking to the right}. However, you then get ``trapped'' inside the loop. If the game is supposed to react to something else as well, or if other game objects need to be updated, this does not work. In that case, it is better to query the keyboard once per frame:
