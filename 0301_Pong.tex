...
\lstsource{SRC/02 Examples/01 Pong/v04/pong04.py}{118}{126}{python}{Pong (\Reqref{req0201Punkte}) -- Constructor of \texttt{Score}}
{srcPong04a}

In this method, the current score is rendered using a font object and then positioned accordingly.

\lstsource{SRC/02 Examples/01 Pong/v04/pong04.py}{134}{136}{python}{Pong (\Reqref{req0201Punkte}) -- \texttt{Score.render()}}
{srcPong04b}

In \texttt{update()}, the appropriate score value is updated and \texttt{render()} is called.

\newpage
\lstsource{SRC/02 Examples/01 Pong/v04/pong04.py}{128}{132}{python}{Pong (\Reqref{req0201Punkte}) -- \texttt{Score.update()}}
{srcPong04c}
...
\subsubsection*{Why is a user-defined event more elegant than direct access?}
...
\begin{itemize}
	...
	The \texttt{Ball} only knows one thing:	\emph{A point was scored by player X.} The \texttt{Game}, on the other hand, decides what that means in practice. Update the score data, re-render the score display, maybe play a sound, reset the ball, or start the next serve.	Each class focuses on its own responsibility \index{SRP}(\randnotiz{SRP}).
...
\textbf{Rule of thumb:} The ball reports what happened and the game decides what to do about it.
...
Rule of thumb: \emph{Pausing does not mean drawing nothing -- it means changing nothing.}
