...
class Bullet(pygame.sprite.Sprite):
    ...
    def update(self, *args: Any, **kwargs: Any) -> None:
        ...
        if self.rect.top > cfg.WINDOW.bottom - 30:
            self.kill()                             # self-destruction (remove sprite)§\label{srcTime0004}§
...
class Game(object):
    ...
    def run(self) -> None:
        ...
        while self.running:
            ...
            self.update()
            self.draw()
            self.clock.tick(cfg.FPS)           # frame rate limiting§\label{srcTime0005}§
            time_current = time()
    ...
    def update(self) -> None:
        self.new_bullet()                           # spawn bullet§\label{srcTime0006}§
