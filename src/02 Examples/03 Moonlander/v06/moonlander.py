...
class Lander:
    def __init__(self, screen: pygame.Surface) -> None:
        ...
        self.velocity = 0                     
        self.fuel_initial = cfg.LEVEL["fair"]       # Starttreibstoff §\label{moonlander0603}§             
    ...
    def draw(self) -> None:
        if self.thrusting:
            ...
            self.screen.blit(self.surface, self.rect.topleft)
        print(self.fuel)                            # Temporary only§\label{moonlander0607}§

    def move(self) -> None:
        if self.thrusting and self.fuel > 0:                     # Fuel remaining?§\label{moonlander0608}§
            self.velocity += cfg.THRUST * cfg.DELTATIME
            self.fuel -= self.fuel_consumption * cfg.DELTATIME   # Fuel consumption§\label{moonlander0606}§
            if self.fuel < 0:
                self.thrusting = False
                self.fuel = 0
