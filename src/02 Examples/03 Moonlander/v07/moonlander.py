...
class Lander:
    ...
    def draw_status(self) -> None:
        ...
        # Text output §\label{moonlander0705}§
        font = pygame.font.SysFont("Consolas", 14)
        labels = "Velocity (px/s):"
        labels += "\nHeight (px):"
        values = f"{self.velocity:>7.0f}"
        values += f"\n{-1*(self.rect.bottom - (cfg.WINDOW.bottom - cfg.HORIZONT)):>7.0f}"
        ...
        # Bar display §\label{moonlander0706}§
        if self.thrusting:
    ...
    def move(self) -> None:
        ...
        if self.rect.bottom >= cfg.WINDOW.bottom - cfg.HORIZONT:
            ...
            self.velocity = 0      # Landing -> velocity 0§\label{moonlander0702}§         
...
class Game:
    ...
    def watch_for_events(self) -> None:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                self.running = False
            elif event.type == pygame.WINDOWCLOSE:  # New due to 2 windows! §\label{moonlander0703}§ 
                self.running = False
    ...
    def restart(self) -> None:
        ...
        self.lander = Lander(self.window)     # due to status window§\label{moonlander0704}§  
        self.running = True
