...
class Pause(pygame.sprite.Sprite):
    def __init__(self, *groups: Tuple[pygame.sprite.Group]) -> None:
        super().__init__(*groups)
        self.rect = pygame.rect.Rect(cfg.WINDOW.topleft, cfg.WINDOW.size)
        self.image = pygame.surface.Surface(self.rect.size).convert_alpha()
        self.image.fill([120, 120, 120, 200])       # Transparent Grey§\label{srcPong0801}§
...
class Help(pygame.sprite.Sprite):
    def __init__(self, *groups: Tuple[pygame.sprite.Group]) -> None:
        ...
        self.rect = pygame.rect.Rect(cfg.WINDOW.topleft, cfg.WINDOW.size)
        self.image = pygame.surface.Surface(self.rect.size).convert_alpha()
        self.image.fill([20, 20, 20, 200])          # Transparent Grey§\label{srcPong0802}§
...
class Ball(pygame.sprite.Sprite):
    def __init__(self, *groups: Tuple[pygame.sprite.Group]) -> None:
        ...
        self.sounds["left"] = pygame.mixer.Sound(cfg.get_sound("playerl.mp3"))
        self.sounds["right"] = pygame.mixer.Sound(cfg.get_sound("playerr.mp3"))
        self.sounds["bounce"] = pygame.mixer.Sound(cfg.get_sound("bounce.mp3"))
    ...
    def vertical_flip(self) -> None:
        if cfg.SOUNDFLAG:
            rel_pos = self.rect.centerx / cfg.WINDOW.width
...
class Score(pygame.sprite.Sprite):
    ...
    def render(self):
        ...
        self.rect = self.image.get_rect(centerx=cfg.WINDOW.centerx, top=15)
...
class Game:
    def __init__(self):
        ...
        self.helping = False                        # Help Flag§\label{srcPong0804}§
    ...
    def update(self):
        if not (self.pausing or self.helping):      # §\label{srcPong0805}§
            self.check_collision()
            for i in cfg.KI.keys():
                if cfg.KI[i]:
    ...
    def draw(self):
        ...
        if self.pausing:                            # §\label{srcPong0806}§
            self.pause.draw(self.screen)
        elif self.helping:                          # §\label{srcPong0807}§
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    cfg.SOUNDFLAG = not cfg.SOUNDFLAG
                    ...
                    cfg.KI["left"] = not cfg.KI["left"]
                    if not cfg.KI["left"]:
                        self.paddle["left"].update(action="halt")
                elif event.key == pygame.K_2:
                    cfg.KI["right"] = not cfg.KI["right"]
                    if not cfg.KI["right"]:
                        self.paddle["right"].update(action="halt")
                elif event.key == pygame.K_p:
                    if not self.helping:
                        self.pausing = not self.pausing  # Toogle pause mode§\label{srcPong0805}§
                elif event.key == pygame.K_h:
                    if not self.pausing:
                        self.helping = not self.helping  # Toogle help mode§\label{srcPong0806}§
    ...
    def paddlecontroler(self, paddle: pygame.sprite.Sprite) -> None:
        ...
        elif paddle.rect.centery < self.ball.rect.centery and paddle.rect.bottom < cfg.WINDOW.bottom - 10:
