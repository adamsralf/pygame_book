...
class Game:
    ...
    def update(self) -> None:
        ...
        self.spawn_bubble()
        self.set_mousecursor()                      # §\label{srcBubble0501}§
    ...
    def collidepoint(self, point: Tuple[int, int], sprite: pygame.sprite.Sprite) -> bool:
        if hasattr(sprite, "radius"):
            ...
            return deltax * deltax + deltay * deltay <= pow(sprite.radius, 2)
        return False
    ...
    def set_mousecursor(self) -> None:
        ...
        for b in self.all_sprites:
            if self.collidepoint(pos, b):           # Mouse pos inside? §\label{srcBubble0502}§
                is_over = True
                break
        if is_over:
