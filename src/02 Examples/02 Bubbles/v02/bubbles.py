...
class Game:
    def __init__(self) -> None:
        ...
        self.all_sprites = pygame.sprite.Group()    # All bubbles§\label{srcBubble0206}§
        self.running = True
    ...
    def spawn_bubble(self) -> None:
        if self.timer_bubble.is_next_stop_reached():
            ...
            for _ in range(100):
                b.randompos()
                b.radius += cfg.DISTANCE       # Distance to other bubbles§\label{srcBubble0207}§
                collided = pygame.sprite.spritecollide(b, self.all_sprites, False, pygame.sprite.collide_circle)
                b.radius -= cfg.DISTANCE       # Old radius!§\label{srcBubble0208}§
                if not collided:
