...
class Game:
    ...
    def watch_for_events(self) -> None:
        for event in pygame.event.get():
            ...
            elif Game.STATUS in ["win", "draw"]:            # §\label{srcTictactoe0402}§
                if event.type == pygame.KEYDOWN:
            ...
            if kwargs.get("action") == "place":
                ...
                if cfg.PLAYGROUND.collidepoint(pos):
                    ...
                    if Game.BOARD[row][col] is None:
                        ...
                        if self.check_winning():                    #§\label{srcTictactoe0401}§
                            Game.STATUS = "win"
                            return
                        else:
                            self.change_player()
    ...
    def draw_start(self) -> None:
        ...
        self.draw_message(welcome, text)    

    def draw_win(self) -> None:
        welcome = f"Player {Game.PLAYER} wins!"
        text = [
    ...
    def draw_message(self, headline: str, message: list[str]) -> None:
        surface = pygame.Surface(cfg.WINDOW.size, pygame.SRCALPHA)
        surface.fill((0, 0, 0, 180))
        headline = Game.FONTS["title"].render(headline, True, cfg.COLORS["TITLE"])
