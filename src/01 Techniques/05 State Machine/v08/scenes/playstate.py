        ...
        for event in events:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_h:
                    pygame.event.post(pygame.event.Event(self.game.EVENTS["HELP"]))   # §\label{srcPlayState0801}§
            ...
            if cfg.PLAYGROUND.collidepoint(pos):
                ...
                if self.game.BOARD[row][col] is None:
                    ...
                    if self.check_winning():  
                        pygame.event.post( pygame.event.Event(self.game.EVENTS["WIN"])) 
                        return
                    else:
                        self.change_player()
            if self.check_ending():
                pygame.event.post(pygame.event.Event(self.game.EVENTS["DRAW"]))    
                return
