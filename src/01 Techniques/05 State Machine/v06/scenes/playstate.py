            ...
            if cfg.PLAYGROUND.collidepoint(pos):
                ...
                if self.game.BOARD[row][col] is None:
                    ...
                    if self.check_winning():  
                        from scenes.winstate import WinState
                        self.game.state = WinState(self.game)   # §\label{srcPlayState0601}§
                        return
                    else:
                        self.change_player()
            if self.check_ending():
                from scenes.drawstate import DrawState
                self.game.state = DrawState(self.game)          # §\label{srcPlayState0602}§
