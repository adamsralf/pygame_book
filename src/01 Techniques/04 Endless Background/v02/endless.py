    ...
    def update(self, *args: Any, **kwargs: Any) -> None:
        ...
        if "action" in kwargs.keys():
            if kwargs["action"] == "toggle_frame":
                ...
                pos_tile1.left = pos_tile1.left % self.bgimage.rect.width   # §\label{srcEndless0201}§
                self.image.fill("Grey")
                self.image.blit(self.bgimage.image, pos_tile1)
                if pos_tile1.left > 0:                                      # §\label{srcEndless0202}§
                    pos_tile2 = pos_tile1.copy()
                    pos_tile2.right = pos_tile1.left
                    self.image.blit(self.bgimage.image, pos_tile2)
                elif pos_tile1.left < 0:                                    # §\label{srcEndless0203}§
                    pos_tile2 = pos_tile1.copy()
