...
\lstsource{SRC/00 Introduction/10 Mouse/example01/mouse.py}{101}{114}{python}{Mouse actions: \texttt{Game.update()} und \texttt{Game.draw()}}{srcMaus00d} 
...
One way to work around this problem is to use \texttt{update()} as a dispatching hub. In the class \texttt{pygame.sprite.Sprite}, this method is defined with the following signature:
...
Two final notes on \texttt{pygame.transform.rotate()}\randnotiz{rotate()}\myindex{pyg}{\texttt{transform}!\texttt{rotate()}|underline}:
\begin{enumerate}
...
\begin{longtable}{ll}
	...
	\texttt{0} &  undefined \\ 
	\texttt{1} &  left mouse button\\ 
	\texttt{2} &  middle mouse button/mouse wheel\\ 
	\texttt{3} &  right mouse button\\ 
	\texttt{4} &  scroll the mouse wheel up\\ 
	\texttt{5} &  Scroll the mouse wheel down\\ 
\end{longtable} 
