...
\begin{itemize}
	...
	\item The approach used here works only because the bitmap represents a ball. In general, you should not repeatedly rotate the already rotated image. Instead, keep the original image unchanged and update only an angle variable. If you rotate the same image, the contents may be scaled up or down depending on the bitmap’s shape, resulting in visual artifacts and severe pixel degradation.
\end{itemize}
\end{warningbox}
