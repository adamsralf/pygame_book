...
\begin{diskbox}
	\url{https://github.com/adamsralf/pygame_book/blob/main/src/00%20Introduction/02%20Primitives/primitives00.py}
\end{diskbox}
...
They do not play a very important role in game programming, because most graphics are predefined bitmaps, but they can be quite useful. For this reason, I~will only introduce a few of them here.

\lstsource{src/00 Introduction/02 Primitives/primitives00.py}{1}{40}{python}{Graphic Primitives}{srcPrimitives00}
...
rectangle. There are several ways to define a rectangle in Pygame. Since we will need it very often later on, I~would like to introduce the class \texttt{pygame.Rect}\myindex{pyg}{\texttt{Rect}}\randnotiz{Rect} here. It is defined by four parameters: the upper-left corner, its width, and its height. In \zeiref{srcPrimitives02}, a rectangle is therefore defined at the position $(10,10)$ with a width of \SI{20}{px} and a height of \SI{30}{px}.
...
\begin{hintbox}[Hint]
	The class \texttt{Rect} is not a drawn rectangle, but merely a container for information that is relevant for a rectangle. More in \kapref[vref]{secRect}.
\end{hintbox}

More general than a rectangle is a \Gls{polygon}. A polygon\index{Graphic primitives!Polygon|underline}\index{Polygon|underline} is a closed chain of lines that is defined in Pygame by its points. Similar to rectangles, there are filled (\zeiref{srcPrimitives06}) and unfilled (\zeiref{srcPrimitives07}) variants. Both are drawn using \texttt{pygame.draw.polygon()}\randnotiz{polygon()}\myindex{pyg}{\texttt{draw}!\texttt{polygon()}}. 

\begin{warningbox}[Warning]
	Be careful with the line thickness: the lines grow outward, which can quickly lead to ugly offsets at the corners. Try it out by changing the value~\texttt{2} to~\texttt{5}.
\end{warningbox}
...
\begin{diskbox}
	\url{https://github.com/adamsralf/pygame_book/tree/main/src/00%20Introduction/02%20Primitives/circles}
\end{diskbox}

\begin{wrapfigure}[12]{r}{5.8cm}%
	\begin{center}%
		\vspace{-1cm}%
		\myfigure{circles01.png}{0.35}{Not a particle swarm}{picCircles01}%
	\end{center}%
\end{wrapfigure}%
...
\lstsource{src/00 Introduction/02 Primitives/circles/v01/circles.py}{1}{99}{python}{Particle swarm, Version 1.0}{srcCircles01}

\begin{wrapfigure}[11]{r}{6.0cm}%
	\begin{center}%
		\vspace{-1.5cm}%
		\myfigure{circles02a.png}{0.8}{Particle swarm\\Version 2}{picCircles02}%
	\end{center}%
\end{wrapfigure}%
...
\lstsource{src/00 Introduction/02 Primitives/circles/v02/circles.py}{7}{11}{python}{Particle swarm, Version 2.0}{srcCircles02}
...
\begin{warningbox}[Why do I~not call \texttt{remove()} already inside the update loop?]
	Because: \emph{Never increase or decrease the length of a list while you are iterating over it.} Very strange effects can occur. 
\end{warningbox}

Try to guess the number of loop iterations of the following program:	
...
\lstsource{src/00 Introduction/02 Primitives/circles/v06/circles.py}{6}{70}{python}{Particle swarm, Version 6.0}{srcCircles06}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Example: Landscape}\label{secLandscape}
\begin{diskbox}
	\url{https://github.com/adamsralf/pygame_book/tree/main/src/00%20Introduction/02%20Primitives/landscape}
\end{diskbox}
...
I~would like to create a small animated landscape that should look like in \abbref[vref]{picLandscape00}. The sun should rise on the left, move across the sky, and set on the right. The blue color of the sky should also change depending on the time of day.
...
How do I~calculate the height now? For the sake of simplicity, I~let the sun follow the first half of the sine function. For this, the domain must be~$[0, \pi]$; this is the hump of the \gls{sinefunction} that lies above the x-axis. The range of the sine function from~$0$ to~$\pi$ is $[0, 1]$. If I~multiply this value by the horizon, I~obtain values from~$0$ to \texttt{horizon}. Finally, I~add the radius so that the sun just touches the upper edge at its highest point.
...
\begin{hintbox}
	...
	Objects drawn later appear in front of earlier ones.
\end{hintbox}
...
\myebild{moonlander00.png}{1.0}{Drawing a Moonlander}{picMoonlander00}