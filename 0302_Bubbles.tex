...
\myebild{aquarium01}{0.15}{Bubbles: background image (aquarium.png)}{picAquarium01}
...
The \texttt{Timer} class is exactly the one described above in \kapref[vref]{secZeitstuerung}\index{Timer}; everything is explained there.
...
The basic idea behind \texttt{spawn\_bubble()} is to keep guessing a position for a new bubble until a free area is found. To avoid ending up in an \gls{endlosschleife}, the number of attempts is limited to~100. If no free area is found, the bubble is not added to the sprite group -- it is simply discarded.
...
\lstsource{SRC/02 Examples/02 Bubbles/v04/bubbles.py}{14}{14}{python}{Bubbles (\reqref{reqBlasenWachstum}.\ref{reqBlasenWachstumMax}) -- extension of \texttt{config.py}}{srcBubbles04b}
...
\lstsource{SRC/02 Examples/02 Bubbles/v04/bubbles.py}{45}{70}{python}{Bubbles (\reqref{reqBlasenWachstum}) -- extension of \texttt{Game.update()}}{srcBubbles04d}  
...
\lstsource{SRC/02 Examples/02 Bubbles/v05/bubbles.py}{126}{129}{python}{Bubbles (\reqref{reqMauscursor}) -- \texttt{update()} in \texttt{Game}}{srcbubbles05c}  
...
Note: As a general rule, implement as little logic as possible in \texttt{watch\_\-for\_\-events()}. This method acts as a dispatcher; the actual processing should always be delegated to separate methods.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\Reqref{reqPunktestand}: Score}\br{Score}{reqPunktestand}
\begin{enumerate}
...
Popping bubbles should, of course, be rewarded with points. To do this, the score has to be calculated and displayed. The simplest way to keep track of the score is to use a static variable in \texttt{Settings} or a global variable. I prefer option~1 (see \srcref[vref]{srcbubbles07a}).
...
\lstsource{SRC/02 Examples/02 Bubbles/v14/bubbles.py}{202}{212}{python}{Bubbles game state -- \texttt{set\_pause()}, \texttt{set\_resume()}, and \texttt{set\_game\_over()} of \texttt{Game}}{srcBubbles14d}  
...
What remains is the helper method that checks whether the \SI{10}{seconds} have elapsed: \texttt{check\_waiting\_timeout()}. In this method, the elapsed time is compared with \texttt{wait\_duration}. If the time has elapsed, the game state is set to \texttt{GAME\_OVER}, so that the game can terminate cleanly.
