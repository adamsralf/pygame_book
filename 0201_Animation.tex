...
\begin{diskbox}
	\url{https://github.com/adamsralf/pygame_book/tree/main/src/01%20Techniques/01%20Animation}
\end{diskbox}
...
The source code in \srcref[vref]{srcAnimation00b} differs from the chapter above (see \secref[vref]{secClassTimer} by only one feature. The \texttt{Timer} class has been extended by the method \texttt{change\_duration()}. This method makes it possible to change the duration of the time interval at runtime, with a lower limit of~\SI{0}{ms}. We will use this feature shortly to manually adjust the animation speed.

\lstsource{SRC/01 Techniques/01 Animation/animation00.py}{8}{26}{python}{The running cat (1), Version 1.0 --  \texttt{Timer}}{srcAnimation00b} 
...
\lstsource{SRC/01 Techniques/01 Animation/animation00.py}{29}{55}{python}{The running cat (2), Version 1.0 --  \texttt{Cat}}{srcAnimation00c} 
...
\lstsource{SRC/01 Techniques/01 Animation/animation00.py}{58}{81}{python}{The running cat (3), Version 1.0 --  Constructor and \texttt{run()}}{srcAnimation00d} 
...
\lstsource{SRC/01 Techniques/01 Animation/animation00.py}{83}{93}{python}{The running cat (4), Version 1.0 --  \texttt{watch\_for\_events()}}{srcAnimation00e} 

The remaining source code (\srcref[vref]{srcAnimation00f}) should be self-explanatory. When you start the program, an animated cat movement will be displayed. Feel free to try changing the animation speed.

\lstsource{SRC/01 Techniques/01 Animation/animation00.py}{97}{106}{python}{The running cat (5), Version 1.0 -- \texttt{update()} and \texttt{draw()}}{srcAnimation00f} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The Class Animation}

As with time control, I~am bothered by the fact that the animation logic is spread across the \texttt{Cat} class, which in my opinion violates the SRP. So let us simply build a dedicated animation class (see \srcref[vref]{srcAnimation01a}).
...
\begin{hintbox}[Question to the audience]
	Why was \texttt{imageindex} initialized to~$-1$ in the constructor?
\end{hintbox}
...
\lstsource{SRC/01 Techniques/01 Animation/animation01.py}{9}{37}{python}{The running cat (6), Version 1.1 -- \texttt{Animation}}{srcAnimation01a} 
...
\lstsource{SRC/01 Techniques/01 Animation/animation01.py}{61}{77}{python}{The running cat (7), Version 1.1 -- \texttt{Cat}}{srcAnimation01b} 
...
\lstsource{SRC/01 Techniques/01 Animation/animation02.py}{62}{84}{python}{The exploding rock (1) -- \texttt{Rock}}{srcAnimation02a} 
...
\newpage
\lstsource{SRC/01 Techniques/01 Animation/animation02.py}{87}{128}{python}{The exploding rock (2) -- \texttt{ExplosionAnimation}}{srcAnimation02b} 

\begin{hintbox}[Hint]
	There is also the source file \texttt{animation03.py}. In this variant, the rocks move and explode when they collide with each other. Take a look!
\end{hintbox}
