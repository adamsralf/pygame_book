...
Now let us look at the whole setup in detail. In the console output above (see page~\pageref{srcConsole00a}), a unique number can be seen for each event, which can be used to identify the event. \randnotiz{custom\_type()}
...
\end{hintbox}
For each user-defined event, such a unique number must be assigned. The simplest approach is to call \texttt{pygame.event.custom\_type()}. You can find corresponding examples in \zeiref{srcEvent0101} and \zeiref{srcEvent0102}.\myindex{pyg}{\texttt{event}!\texttt{custom\_type()}|underline}
...
\begin{hintbox}[Best practice]
	Define all custom event IDs centrally, for example in a configuration file.\myindex{pyg}{\texttt{event}!\texttt{custom\_type()}}
\end{hintbox}

\lstset{firstnumber=1}
\begin{lstlisting}
BUTTON_EVENT   = pygame.event.custom_type()
OVERFLOW_EVENT = pygame.event.custom_type()
NEWPARTICLES   = pygame.event.custom_type()
\end{lstlisting}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Avoid Manually Set Event IDs}

\begin{warningbox}[Bad practice]
	If you use the method shown below, event IDs may be assigned multiple times when they are defined across different source files. It is therefore preferable to use \texttt{pygame.event.custom\_type()} instead.\myindex{pyg}{\texttt{USEREVENT}}
...
\begin{itemize}
	...
	\item \texttt{pygame.event.custom\_type()}:
	\myindex{pyg}{\texttt{event}!\texttt{custom\_type()}}\\
	\url{https://pyga.me/docs/ref/event.html#pygame.event.custom_type}
