...
\lstsource{SRC/00 Introduction/07 Fonts/textoutput00.py}{5}{31}{python}{Simple text output using fonts}{srcFonts00aa} 

But how does the contructor of \texttt{Font} know, which font I want? In \zeiref{textoutput00a} we use the predefined default font. \texttt{pygame.font.SysFont()}\randnotiz{SysFont()}\myindex{pyg}{\texttt{font}!\texttt{SysFont()}} 
takes either the name of a font or \texttt{None} as the first argument. In the latter case, the default font is used. Since Pygame always ships with a default font, this call will always return a valid font object. However, there is no guarantee that it will always be the same font, as the default font itself could change.

The method \texttt{pygame.font.get\_default\_font()}\randnotiz{get\_default\_font()}\myindex{pyg}{\texttt{font}!\texttt{get\_default\_font()}} returns a unique string -- the internal name of the font. You can find in \secref[vref]{secFontInstalled} a program, which shows you all installed fonts and there internal names.
...
\begin{itemize}
	...
	\item \texttt{pygame.font.SysFont()}:
	\myindex{pyg}{\texttt{font}!\texttt{SysFont()}}\\ \url{https://pyga.me/docs/ref/font.html#pygame.font.SysFont}
